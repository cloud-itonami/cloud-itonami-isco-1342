(ns healthsvc.actor
  "HealthServicesManagersActor — the ISCO-08 1342 community healthsvc actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one healthsvc operation request
  ...
  The unconditional invariant: the HealthServicesManagersAdvisor can never
  directly commit a record the HealthServicesManagersGovernor refuses —
...
(defn build-graph
  "Build a compiled HealthServicesManagersActor graph. `store` implements
  `healthsvc.store/Store`. `advisor` implements
  `healthsvc.advisor/Advisor` (defaults to `mock-advisor`).
